...
func codeErrors(at, depth int) error {
	if at >= depth {
		return New(unknownCoder.Code(), "co error")
	}
	return codeErrors(at+1, depth)
}
...
func BenchmarkPkgFormatting(b *testing.B) {
	type run struct {
		stack  int
		format string
	}
